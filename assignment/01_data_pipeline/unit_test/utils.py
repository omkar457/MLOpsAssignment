...
def build_dbs():
    ...
    SAMPLE USAGE
        build_dbs()
    '''
    db_file_path = DB_PATH + UNIT_TEST_DB_FILE_NAME
...
def load_data_into_db():
    ...
    lead_scoring_raw_data_path = DATA_DIRECTORY + 'leadscoring_test.csv'
    try:
        db_conn = sqlite3.connect(db_file_path)
        ...
        ls_data.to_sql(name='loaded_data_test_case', con=db_conn, if_exists='replace', index=False)
    except Error as e:
            print(e)
            return "Error"
    finally:
...
def map_city_tier():
    ...
    try:
        ...
        print(f"Read the data from the 'loaded_data_test_case' table in database file location: {db_file_path}")
        loaded_data = pd.read_sql('SELECT * FROM loaded_data_test_case', con=db_conn)
        ...
        loaded_data.to_sql(name='city_tier_mapped_test_case', con=db_conn, if_exists='replace', index=False)
...
def map_categorical_vars():
    ...
    try:
        ...
        print(f"Read the data from the 'city_tier_mapped_test_case' table in database file location: {db_file_path}")
        loaded_data = pd.read_sql('SELECT * FROM city_tier_mapped_test_case', con=db_conn)
        ...
        df.to_sql(name='categorical_variables_mapped_test_case', con=db_conn, if_exists='replace', index=False)
...
def interactions_mapping():
    ...
    try:
        ...
        loaded_data = pd.read_sql('SELECT * FROM categorical_variables_mapped_test_case', con=db_conn)
...
#--------------------------
        ...
        df_pivot.to_sql(name='interactions_mapped_test_case', con=db_conn, if_exists='replace', index=False)
